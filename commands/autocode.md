...
2. If `model_routing.<agent>` is an object without `reasoning` → auto-detect from the model name (same heuristic as for strings below)
3. If `model_routing.<agent>` is a string → default by model tier:
   - `reasoning: true` if model name is "opus" (extended thinking by default)
   - `reasoning: false` otherwise (sonnet, haiku)
4. If `model_routing.<agent>` is not set → use defaults (sonnet = non-reasoning, opus = reasoning)

Note: JSON Schema defaults in the manifest are not applied automatically at runtime; the orchestrator must implement defaults explicitly as above.
...
**Layer 1 — ROLE**: Load the agent's `.md` file from `agents/<agent>.md`. This defines identity, capabilities, and write constraints.
...
- Body: Use the Reviewer's suggested PR body (if approved), otherwise include Builder's summary. Always end with `🤖 Generated by [AutoCode](https://github.com/JustinBeaudry/autocode)`
- Labels: `autocode`
...
🤖 Addressed by [AutoCode](https://github.com/JustinBeaudry/autocode)
```
...
2. These self-reported violations use `severity: "warning"` and the standard warning multiplier (1)
